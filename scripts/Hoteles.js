class Hoteles extends React.Component {
  ...
  componentDidMount(){
    this.setState({initialDate: new Date(Math.min.apply(null,hotelsData.map(e=>  new Date(e.availabilityFrom)))).toLocaleDateString()})
    this.setState({endDate: new Date(Math.max.apply(null,hotelsData.map(e=>  new Date(e.availabilityTo)))).toLocaleDateString()})
  }
  ...
  handleChange = (event) =>{
    ...
    if(name == "EndDate")
      this.setState({endDateFormat : moment(value)});
  ...
  render(){
    const message =  `No hay resultados para la búsqueda realizada ... 
      Fecha inicial: ${this.state.initialDateFormat == ""? "Seleccione fecha ": moment(this.state.initialDateFormat).locale('es').format('dddd, DD MMMM YYYY')} ,
      Fecha final: ${this.state.endDateFormat == ""? "Seleccione fecha ": moment(this.state.endDateFormat).locale('es').format('dddd, DD MMMM YYYY')}, 
    ...
    return (
      <div>
        <Header
          initialDate= {this.state.initialDateFormat == ""? "Seleccione fecha ":moment(this.state.initialDateFormat).locale('es').format('dddd, DD MMMM YYYY ') }
          endDate= {this.state.endDateFormat == ""? "Seleccione fecha ":moment(this.state.endDateFormat).locale('es').format('dddd, DD MMMM YYYY ') }
        />
